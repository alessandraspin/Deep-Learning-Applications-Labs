...
def get_cifar100_loaders(config):
    print("Caricamento dei dati CIFAR-100...")
    ...
    ds_train_full = CIFAR100(root='./data', train=True, download=True, transform=transform_train)
    ds_val_full = CIFAR100(root='./data', train=True, download=True, transform=transform_test)
    ds_test = CIFAR100(root='./data', train=False, download=True, transform=transform_test)
    ...
    print("CIFAR-100 DataLoaders pronti.")
    print(f"  - Training samples: {len(ds_train)}, Validation samples: {len(ds_val)}, Test samples: {len(ds_test)}")
    return train_loader, val_loader, test_loader